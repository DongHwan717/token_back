...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private final MemberRepository memberRepository; // 유저 정보 가져오기 위함 (예시)
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        Optional<MemberEntity> userOptional = memberRepository.findBySocialIdAndProvider(socialId);
        MemberEntity memberEntity = userOptional.orElseThrow(() -> new IllegalArgumentException("User not found: " + socialId)); // 예외 처리

        // 사용자 정보로 JWT 토큰 생성
        String accessToken = jwtUtil.createToken(Map.of("mno", memberEntity.getMno().toString(), "role", memberEntity.getRole()), 10);
        String refreshToken = jwtUtil.createToken(Map.of("mno", memberEntity.getMno().toString()), 60 * 24 * 7);
